...
data = preprocessed_rumour_events + preprocessed_nonrumour_events
classes = ["Rumour"] * len(preprocessed_rumour_events) + ["Nonrumour"] * len(preprocessed_nonrumour_events)

# First split out training data
trn__dev_tst__sss = StratifiedShuffleSplit(n_splits=1, train_size=trn_size)
trn_index, dev_tst_index = [
    (trn_index, dev_tst_index)
    for trn_index, dev_tst_index
    in trn__dev_tst__sss.split(data, classes)
][0]

trn_data = []
trn_classes = []
for i in trn_index:
    trn_data.append(data[i])
    trn_classes.append(classes[i])

dev_tst_data = []
dev_tst_classes = []
for i in dev_tst_index:
    dev_tst_data.append(data[i])
    dev_tst_classes.append(classes[i])


# Then split the rest into development and test data
dev__tst__sss = StratifiedShuffleSplit(n_splits=1, train_size=tst_size / (dev_size + tst_size))
dev_index, tst_index = [
    (dev_index, tst_index)
    for dev_index, tst_index
    in dev__tst__sss.split(dev_tst_data, dev_tst_classes)
][0]
...
dev_classes = []
for i in dev_index:
    dev_data.append(dev_tst_data[i])
    dev_classes.append(dev_tst_classes[i])

tst_data = []
tst_classes = []
for i in tst_index:
    tst_data.append(dev_tst_data[i])
    tst_classes.append(dev_tst_classes[i])
...
tuned_lr_clf = LogisticRegression(C=1.0, tol=0.0001)
